...
extern vector<GridPosition> robotLocation;
extern vector<GridPosition> boxLocation;
extern vector<GridPosition> doorLocation;
...
void displayGridPane(void)
{
	...
	for (int i = 0; i < numBoxes; i++)
	{
		...
		drawRobotAndBox(i, robotLocation[i].row, robotLocation[i].col, boxLocation[i].row, boxLocation[i].col, doorAssign[i]);
	...
	for (int i = 0; i < numDoors; i++)
	{
		...
		drawDoor(i, doorLocation[i].row, doorLocation[i].col);
		//std::cout << "i'm right after the drawdoor call in frontend.cpp" << std::endl;
	}
...
void displayTextualInfo(const char *infoStr, int xPos, int yPos, FontSize fontSize)
{
	...
	//-----------------------------------------------
	int textWidth = 0;
	(void)textWidth; // suppresses the warning for this variable
	switch (fontSize)
	{
