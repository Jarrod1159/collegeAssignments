...
//	Piece of advice, wheSnever you do a grid-based (e.g. image processing),
...
//	Yep to what's below.
	...
	for (int k = 0; k < numThreads; k++)
	{
		...
		int errCode = pthread_create(&info[k].threadID, nullptr, threadFuncV2, info + k);
		numLiveThreads++;
		displayStatePane();
		if (errCode != 0)
		{
...
//---------------------------------------------------------------------
void* threadFuncV2(void* arg)
{
    ThreadInfo* data = static_cast<ThreadInfo*>(arg);

while (true) {
    // Acquire the lock before calling oneThreadGeneration()
    ...
    if (allThreadsFinished) 
	{
        ...
        // Reset isFinished for all threads before releasing the lock
        for (int i = 0; i < numThreads; i++) 
		{
